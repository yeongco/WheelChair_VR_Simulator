...
namespace Autohand {
    public class MetaHandPoseGestureData {
    ...
    public class MetaHandGestureEvent : HandPoseDataContainer {
        [Header("Gesture Event Settings")]
        public List<MetaXRAutoHandTracking> trackingHands;
        ...
        public UnityEvent<Hand, MetaHandPoseGestureData> OnGestureStartEvent;
        public UnityEvent<Hand, MetaHandPoseGestureData> OnGestureStopEvent;


        Dictionary<Hand, MetaHandPoseGestureData> currentHandGestureData = new Dictionary<Hand, MetaHandPoseGestureData>();
        int lastHandCount = 0;
        ...
        public bool IsTrackingActive() {
            foreach(MetaXRAutoHandTracking hand in trackingHands) {
                if(hand.hand == null)
                    return false;
        ...
        public MetaHandPoseGestureData GetCurrentGestureData(Hand hand) {
        ...
        protected void CheckForHandListChanges() {
            if (trackingHands.Count != lastHandCount) {
                ...
                foreach(MetaXRAutoHandTracking hand in trackingHands) {
                    currentHandGestureData.Add(hand.hand, new MetaHandPoseGestureData());
                }
            }
        }
        ...
        protected virtual void UpdatePoseDifferenceValues() {
            ...
            foreach (MetaXRAutoHandTracking handTracking in trackingHands) {
                var hand = handTracking.hand;
                if (hand == null || (hand.left && !leftPose.isSet) || (!hand.left && !rightPose.isSet)) 
                    continue;

                MetaHandPoseGestureData gestureData = currentHandGestureData[hand];
        ...
        protected bool IsValidPoseState(Hand hand, ref MetaHandPoseGestureData gestureData) {
        ...
        protected virtual void OnGestureStart(Hand hand, ref MetaHandPoseGestureData gestureData) {
        ...
        protected virtual void OnGestureStop(Hand hand, ref MetaHandPoseGestureData gestureData) {
            if(gestureData.currentPoseState) {
        ...
        protected virtual bool IsValidActivationPoseDirection(Hand hand, ref MetaHandPoseGestureData data) {
            if(!IsTrackingActive())
                return false;
        ...
        protected virtual bool IsValidPoseDirection(Hand hand, ref MetaHandPoseGestureData data) {
            if(!IsTrackingActive())
                return false;
